...
ssh root@vps0048.publiccloud.com.br "pkill gunicorn; sudo rm -rf /vagrant; mkdir /vagrant; cd /vagrant; git clone -b build$1 https://github.com/marcoprado17/crescer-saudavel-2.git .; scripts/server_setups/production_server_setup.sh; python scripts/server_setups/restart_db.py; python scripts/server_setups/fill_db.py; scripts/start_server/start_production_server.sh"
