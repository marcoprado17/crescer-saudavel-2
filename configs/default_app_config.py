# !/usr/bin/env python
# -*- coding: utf-8 -*-
# ======================================================================================================================
# Created at 22/12/16 by Marco Aurélio Prado - marco.pdsv@gmail.com
# ======================================================================================================================
from os.path import join

DEBUG = False

STATIC_FOLDER = "static"
STATIC_FULL_PATH = "/vagrant/build/static"
IMAGES_FOLDER = "imgs"
IMAGES_FROM_STATIC_PATH = join(STATIC_FOLDER, IMAGES_FOLDER)
IMAGES_FULL_PATH = join(STATIC_FULL_PATH, IMAGES_FOLDER)
PRODUCT_IMAGES_FOLDER = "product"
PRODUCT_IMAGES_FROM_STATIC_PATH = join(IMAGES_FROM_STATIC_PATH, PRODUCT_IMAGES_FOLDER)
PRODUCT_IMAGES_FULL_PATH = join(IMAGES_FULL_PATH, PRODUCT_IMAGES_FOLDER)
BLOG_THUMBNAIL_IMAGES_FOLDER = "blog-thumbnail"
BLOG_THUMBNAIL_IMAGES_FROM_STATIC_PATH = join(IMAGES_FROM_STATIC_PATH, BLOG_THUMBNAIL_IMAGES_FOLDER)
BLOG_THUMBNAIL_IMAGES_FULL_PATH = join(IMAGES_FULL_PATH, BLOG_THUMBNAIL_IMAGES_FOLDER)
CAROUSEL_IMAGES_FOLDER = "carousel"
CAROUSEL_IMAGES_FROM_STATIC_PATH = join(IMAGES_FROM_STATIC_PATH, CAROUSEL_IMAGES_FOLDER)
CAROUSEL_IMAGES_FULL_PATH = join(IMAGES_FULL_PATH, CAROUSEL_IMAGES_FOLDER)
OTHER_IMAGES_FOLDER = "other"
OTHER_IMAGES_FROM_STATIC_PATH = join(IMAGES_FROM_STATIC_PATH, OTHER_IMAGES_FOLDER)
OTHER_IMAGES_FULL_PATH = join(IMAGES_FULL_PATH, OTHER_IMAGES_FOLDER)
MORE_CATEGORIES_IMAGES_FOLDER = "more-categories"
MORE_CATEGORIES_FROM_STATIC_PATH = join(IMAGES_FROM_STATIC_PATH, MORE_CATEGORIES_IMAGES_FOLDER)
MORE_CATEGORIES_IMAGES_FULL_PATH = join(IMAGES_FULL_PATH, MORE_CATEGORIES_IMAGES_FOLDER)
TAG_IMAGES_FOLDER = "tag"
TAG_IMAGES_FROM_STATIC_PATH = join(IMAGES_FROM_STATIC_PATH, TAG_IMAGES_FOLDER)
TAG_IMAGES_FULL_PATH = join(IMAGES_FULL_PATH, TAG_IMAGES_FOLDER)
BLOG_CONTENT_IMAGES_FOLDER = "blog-content"
BLOG_CONTENT_IMAGES_FROM_STATIC_PATH = join(IMAGES_FROM_STATIC_PATH, BLOG_CONTENT_IMAGES_FOLDER)
BLOG_CONTENT_IMAGES_FULL_PATH = join(IMAGES_FULL_PATH, BLOG_CONTENT_IMAGES_FOLDER)

LOGGING_FORMAT = "[ %(levelname)8s | %(asctime)s ] - [ %(pathname)64s | %(funcName)32s | %(lineno)4d ] - %(message)s"
LOGGING_FILENAME = "/vagrant/logs/log"
LOGGING_WHEN = 'D'
LOGGING_INTERVAL = 7
LOGGING_BACKUP_COUNT = 4

SQLALCHEMY_TRACK_MODIFICATIONS = True
SQLALCHEMY_NATIVE_UNICODE = True

MAIL_SERVER = "smtp.gmail.com"
MAIL_PORT = 465
MAIL_USERNAME = "crescer.saudavel.suporte@gmail.com"
MAIL_USE_TLS = False
MAIL_USE_SSL = True

PRODUCT_IMAGE_WIDTH = 600
PRODUCT_IMAGE_HEIGHT = 600
BLOG_THUMBNAIL_IMAGE_WIDTH = 900
BLOG_THUMBNAIL_IMAGE_HEIGHT = 500
CAROUSEL_IMAGE_WIDTH = 2560
CAROUSEL_IMAGE_HEIGHT = 500
MORE_CATEGORIES_IMAGE_WIDTH = 512
MORE_CATEGORIES_IMAGE_HEIGHT = 512
TAG_IMAGE_WIDTH = 128
TAG_IMAGE_HEIGHT = 128
BLOG_THUMBNAIL_WIDE_IMAGE_WIDTH = 540
BLOG_THUMBNAIL_WIDE_IMAGE_HEIGHT = 160

# WHOOSH_BASE variable name can't be renamed. It's used by whoosh library like it is.
WHOOSH_BASE = "/vagrant/whoosh_index"
